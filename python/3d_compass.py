...
compass = context.socket(zmq.SUB)
compass.connect("tcp://urpobotti.local:7574")
compass.setsockopt_string(zmq.SUBSCRIBE, u'')

poller = zmq.Poller()
#poller.register(pinger, zmq.POLLIN)
#poller.register(encoder, zmq.POLLIN)
poller.register(compass, zmq.POLLIN)

compass_v = [0, 0, 0] # North/South, East/West, Up/Down
...
#			print tires
	...
	for i in range(0,3):
		if compass in socks and socks[compass] == zmq.POLLIN:
			msg = compass.recv_multipart()
			if msg[0] == 'compass_x':
				compass_v[0] = float(msg[1])
			if msg[0] == 'compass_y':
				compass_v[1] = float(msg[1])
			if msg[0] == 'compass_z':
				compass_v[2] = float(msg[1])
	#make the compass vector a unit vector
	compass_len = math.hypot(math.hypot(compass_v[0], compass_v[1]),compass_v[2])
	compass_v[0] /= compass_len
	compass_v[1] /= compass_len
	compass_v[2] /= compass_len
...
# compass
	glColor4f(1.0, 0.0, 0.0, 1.0)
	glPushMatrix()
	glTranslatef(compass_v[0], compass_v[1], compass_v[2])
	glutSolidCube(0.2)
	glPopMatrix()
